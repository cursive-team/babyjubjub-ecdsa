...
use nova_snark::{CompressedSNARK, ProverKey, VerifierKey, provider, PublicParams};
...
pub fn chunk_params(r1cs: &String, output: &String) { 
    ...
    let params_output_folder = root.clone().join(output.clone()).join("params");
    ...
    let params_json = serde_json::to_string(&public_params).unwrap();
    let full_params_path =  root.clone().join(output.clone()).join("public_params.json");
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        let chunk_path = params_output_folder.join(format!("params_{}.gz", i));
