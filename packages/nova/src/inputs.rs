...
pub fn get_example_input() -> HashMap<String, Value> {
    ...
    inputs.insert(
        String::from("s"),
        json!("1556192236082850800011477753789706164136184180458744644984084897070345066570"),
    );
    inputs.insert(
        String::from("root"),
        json!("1799182282238172949735919814155076722550339245418717182904975644657694908682"),
    );
    inputs.insert(
        String::from("Tx"),
        json!("11796026433945242671642728009981778919257130899633207712788256867701213124641"),
    );
    inputs.insert(
        String::from("Ty"),
        json!("14123514812924309349601388555201142092835117152213858542018278815110993732603"),
    );
    inputs.insert(String::from("Ux"), json!("0"));
    inputs.insert(String::from("Uy"), json!("1"));
    inputs.insert(
        String::from("pathIndices"),
        json!(["0", "1", "0", "0", "0", "0", "0", "0"]),
    );
    let siblings = [
...
impl<const D: usize> Membership<D> {
    pub fn to_inputs(&self) -> HashMap<String, Value> {
        ...
        inputs.insert(String::from("Tx"), json!(self.tx));
        inputs.insert(String::from("Ty"), json!(self.ty));
        inputs.insert(String::from("Ux"), json!(self.ux));
        inputs.insert(String::from("Uy"), json!(self.uy));
        let path_indices: Vec<String> = self.path_indices.iter().map(|s| s.to_string()).collect();
        inputs.insert(String::from("pathIndices"), json!(path_indices));
        inputs.insert(String::from("siblings"), json!(self.siblings));
        inputs.insert(
            String::from("sigNullifierRandomness"),
            json!(self.sig_nullifier_randomness),
        );
        inputs.insert(
            String::from("pubKeyNullifierRandomness"),
            json!(self.pub_key_nullifier_randomness),
        );
        inputs
    }
}
